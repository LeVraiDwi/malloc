...
void    *ft_add_page(t_zone *zone, size_t block_size) {
    ...
    if (zone->page == NULL) {
        ...
        if (block_size <= TINY_BLOCK_MAX) 
            ...
            zone->page = ft_alloc_page(SMALL_PAGE_SIZE, Small);
         else 
        ...
        if (block_size <= TINY_BLOCK_MAX) 
            act_page->next = ft_alloc_page(TINY_PAGE_SIZE, Tiny);
        else if (block_size <= SMALL_BLOCK_MAX)
            act_page->next = ft_alloc_page(SMALL_PAGE_SIZE, Small);
         else 
            act_page->next = ft_alloc_page(LARGE_PAGE_SIZE(block_size), Large);
        if (act_page->next == NULL)
            return NULL;
